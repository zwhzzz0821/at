...
static inline double getCpuUsage() {
    CpuTimes prevTimes, currTimes;

    if (!readCpuTimes(prevTimes)) return -1.0;
    usleep(100000); // Sleep for 100ms to get a sample over time
    ...
    if (totalDiff == 0) {
        return 0.0; // Avoid division by zero, can happen if sampling too fast or system is stuck
    }
...
static inline double getMemoryUsage() {
    ...
    if (memTotal == 0) {
        std::cerr << "Error: MemTotal not found or zero in /proc/meminfo" << std::endl;
        return -1.0;
    }

    // Used memory = Total - Free - Buffers - Cached (these are considered "available" for new programs)
